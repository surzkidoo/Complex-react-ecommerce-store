...
const Sidebar = (props) => {
 ...
 const [showLoader,setShowLoader] = useState(false);
  ...
  useEffect(() => {
      ...
      props.setfilter(product.filter((item)=>{
      
        return (sradio? item.product_rating===sradio: true) && item.product_category===scat && (sbrands.length > 0 ? [...sbrands].includes(item.product_brand): true ) && !(Object.getOwnPropertyNames(feature).map(name=>feature[name]?.length > 0? [...feature[name]]?.includes(item.product_feature[name]) : true)).includes(false)
      }))
  ...
  const filterAll = (sub,brand,feature,price,Radio)=>{
    
  }

 
  return showLoader ? <Loader/> : (
    <div className="category-sidebar" style={{display: props.openFilter && "block"}}>
            ...
            {props.sub.map((cat,index) => {
              let item =
                cat.subcategory.length === 0 ? (
                  <div className="sub-category" key={index}>{cat.category_name}</div>
                ) : (
                  ...
                  <div className="sub-sub">
                  {
                    cat.subcategory.map((sub)=>{
                     return  <li className="sub-category" style={{color:sub.category_id==scat && 'rgb(250, 10, 122)'}}  onClick={()=>handleSubCategory(sub.category_id)}>{sub.category_name} </li>
                    })
                  }
                  </div>
                  </div>
      ...
      <div className="brand-filter-container">
        ...
        <div className="toggler-handler">
          <div className="brand-input-container">
            <input type="text" placeholder="Brand" value={brands} onChange={handleBrandSearch} className="brand-input" />
          </div>
              ...
              return (
                <div className="brand-checklist-item" key={index}>
                  <div className="checklist-bg">
                    <div className="brand-checklist-input" style={{backgroundColor:sbrands.includes(brn) && 'rgb(250, 10, 122)'}} onClick={()=>handlebrand(brn)}></div>
                  </div>
...

      {
        props.features.map((featured,index)=>{
          ...
          return <div key={index} className="brand-filter-container">
          <div className="filter-header">
            ...
            <div className="brand-checklist-container">
              {
                featured.value.map((feat,index)=>{
                  return   <div key={index} className="brand-checklist-item">
                  <div className="checklist-bg">
                    <div className="brand-checklist-input" style={{backgroundColor:feature[featured.name]?.includes(feat) && 'rgb(250, 10, 122)'}}  onClick={(e)=>handleFeatures(featured.name,feat)}>
                      {
        ...
        <div className="toggler-handler">
          <div className="brand-checklist-container">
            <div className="brand-checklist-item">
              <div className="checklist-bg">
                <div className="brand-radio-input" style={{backgroundColor:sradio==='1' && 'rgb(250, 10, 122)' }}  onClick={()=>setsradio('1')}></div>
              </div>
              <div className="brand-checklist-name">1</div>
            </div>
            <div className="brand-checklist-item">
              <div className="checklist-bg">
                <div className="brand-radio-input" style={{backgroundColor:sradio==='2' && 'rgb(250, 10, 122)' }} onClick={()=>setsradio('2')}></div>
              </div>
              <div className="brand-checklist-name">2</div>
            </div>
            <div className="brand-checklist-item">
              <div className="checklist-bg">
                <div className="brand-radio-input" style={{backgroundColor:sradio==='3' && 'rgb(250, 10, 122)' }} onClick={()=>setsradio('3')}></div>
              </div>
              <div className="brand-checklist-name" >3</div>
            </div>
            <div className="brand-checklist-item">
              <div className="checklist-bg">
                <div className="brand-radio-input" style={{backgroundColor:sradio==='4' && 'rgb(250, 10, 122)' }} onClick={()=>setsradio('4')}></div>
              </div>
